...
9. Costos esperados. Tokens estimados de entrada y de salida, y costo por request. **La moneda la declara la categoría 09, que se emite cuatro fases después**: hasta entonces el costo se expresa en la moneda que el intake declare. Si el intake no la declara, **es un dato que falta y no una referencia colgada** —por eso no se resuelve con la forma de `Root-Rules.md` §12.1— y se difiere con la de **§12.2**, cuyo evento de cierre se nombra solo: el artefacto de la categoría 09 que fija la moneda. **Declarar la magnitud «pendiente» en prosa no alcanza**: sin la forma no es contable, y la comprobación 6 de `Master-Prompt.md` §10.0 la levanta como hallazgo P1.
...
| --- | --- | --- | --- |
| Prompt sin contrato de salida formal | La salida no es parseable y rompe el consumidor | Forzar JSON con JSON Schema o markdown con headings fijos | [enumerable] |
| Prompt sin few-shot cuando la tarea lo requiere | El modelo no tiene referencia de calidad esperada y alucina | Mínimo dos few-shot por prompt; tres si es generación abierta | [enumerable] |
| Política de uso sin sección de datos sensibles | Se exponen datos personales al proveedor sin control | Sección §2 obligatoria con clasificación de datos y política de redacción | [enumerable] |
| Evaluación sin dataset versionado | No hay forma de comparar versiones del prompt | Versionar `dataset-evaluacion` con sufijo `.md` | [enumerable] |
| Vendor lock-in sin alternativa documentada | Si el proveedor sube precios o cambia política, no hay plan B | Sección §7 de la política con alternativa primaria y plan de portabilidad | [enumerable] |
| Confiar ciegamente en la salida del LLM | Efectos críticos aplicados sin validación humana | Declarar bucle de control humano para tareas con efecto irreversible | [interpretativo] |
| Prompt monolítico con varias tareas | Difícil de evaluar y de versionar | Chain of prompts: un archivo por tarea con contrato propio | [interpretativo] |
| Métricas sin umbral numérico | "Debe ser buena" no es evaluable | Cada métrica con umbral cuantitativo y acción si no se alcanza | [enumerable] |
...
| Casing inconsistente en el slug | Rompe automatizaciones | Forzar Título-Con-Guiones estricto | [interpretativo] |
...
- ¿La unidad de entrega declara `usa_llm = true` en el PRODUCT-INTAKE §17 P.11 o una NFR de IA en §17 P.10? Sin esa declaración, no se genera la categoría.
...
Los ejemplos son ilustrativos y de dominios distintos al material fuente. No deben copiarse literalmente a ninguna unidad de entrega.
...
```text
Sos un {{ESPECIALIDAD-VARIANTE}} responsable de la categoría 04 (prompts AI) de la unidad de entrega {{NOMBRE_UNIDAD_ENTREGA}}.
...
Salida: SDD/Docs/Unidades-Entrega/{{NOMBRE_UNIDAD_ENTREGA}}/04-Prompts-AI/<estructura>.
```
