...
| --- | --- | --- | --- |
| **Método** | Criterio de aceptación del artefacto. Es lo que hace que el entregable sirva, independientemente de con qué se lo construya | Los cuatro estados por superficie; WCAG 2.2 AA; fuente única de datos de ejemplo (`Maqueta-Rules.md` §4.2 a §4.7) | **No.** Desplazarlo baja la calidad, no la adapta |
| **Decisión de stack** | Una elección razonada entre alternativas legítimas. Es del framework porque alguien tenía que elegir, no porque sea la única correcta | JavaScript vanilla, Bootstrap por CDN, sin proceso de build (`Maqueta-Rules.md` §4.1, rotulada como tal desde el conjunto 13.4) | **Sí**, declarándolo. Es sustitución, no desviación |
