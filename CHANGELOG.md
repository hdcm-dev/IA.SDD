...
### Agregado — `Expedientes/0002-Conocimiento-Bundle-JS-En-Solucion-Dotnet/`

El caso que produjo el documento de conocimiento **`Bundle-JS`** —un bundle JavaScript/TypeScript y su capa de interop dentro de una solución .NET con Blazor, con los dos anfitriones que lo verifican—, pedido por el Product Owner en `IA.SDD.Documentacion/PROMPTs/SDD/Catalogado/02-…/` y **entregado fuera de este repositorio a propósito**: `Knowledge-Bundle-JS.md` 1.0 vive en `OUTPUTs/` de ese tool-prompt, con su fila de índice propuesta y la adopción en tres pasos para el fork que lo quiera; acá no hay fila en `Index-Knowledge.md` ni copia en `Conocimiento/`. Dieciséis folios y doce piezas de evidencia con dos manifiestos:

- **001 a 002**: presentación y convocatoria de la mesa, con contrato de entrada, compuerta mecánica (`ev-06`: todo archivo del framework citado por el documento de entrada resuelve) y panel de ocho comisiones —núcleo N1 a N3, verticales V1 a V3 y dos ad hoc: ingeniería del bundle e interoperabilidad Blazor—, con las diez cartas asentadas **antes** de cada despacho (`ev-07`, con manifiesto propio).
- **003 a 010**: los ocho informes, cada uno con hallazgos anclados a línea sobre la norma 13.17, el documento de entrada y un precedente real fijado a commit (`ev-03`). Entre lo que encontraron: el documento de entrada estaba escrito contra 13.15 y no veía lo que 13.16 fijó el mismo día; el precedente no probaba su bundle sin anfitrión .NET contra el artefacto construido, y un módulo ES no abre desde `file://`; el registro de estáticos en el primer build desde limpio, **defecto reproducido y corrección probada** sobre una copia (`ev-08`, SDK 10.0.400); y que `Relevar-Conocimiento.md`, nombrado por el índice y el README de `Conocimiento/`, no existe.
- **011 a 013**: consolidación con plan compuesto, refutación con dieciséis ataques y **jurado de cinco funciones** que hizo proceder quince de ellos, devolvió dos severidades a su valor, agregó la contradicción X-5 y elevó **`ESC-001`**: publicar el expediente pondría el interior de un producto privado en un repositorio público.
- **014**: el dictamen que especifica el documento sección por sección, con seis deudas declaradas —dos artefactos de una misma construcción, PoC HTML servida, API de terceros afuera, documento de entrada superado, WebAssembly y Auto sin verificar, accesibilidad no convocada— y el default de no publicar mientras la escalada estuviera abierta.
- **015**: verificación del documento contra el dictamen, con los esqueletos de §5 construidos de punta a punta (`ev-10`: Node 22, esbuild 0.25.12, TypeScript 5.9.3, SDK .NET 10) y ocho defectos corregidos; **dos criterios se forzaron a fallar y fallaron**.
- **016**: la respuesta del Product Owner a `ESC-001`, **opción B**, y la **redacción S2** que la ejecuta antes del primer push, con la línea `Redacción S2:` de `Expediente-Rules.md` §4: el nombre del producto, su repositorio, sus proyectos, su componente, su prefijo CSS y las rutas del host pasan a marcadores (`<F1>`, `<repo-privado-F1>`, `<Componente>`, `<workspace>`, `<scratchpad>`), descriptos y no transcriptos; huellas antes y después de cada pieza de evidencia; manifiestos regenerados; y los efectos colaterales dichos —seis guiones ya no corren tal cual, y la lista de términos de R-N2-03 quedó redactada sobre sí misma—. `ev-12` comprueba cero rutas del host y cuenta los marcadores. Lo que **no** se redactó también se declara: nombres de archivos internos sin el nombre del producto, líneas citadas y cifras medidas son concepto y evidencia, no identidad.

**Forma provisoria, declarada en su `README.md`.** El expediente se abrió con la forma que dejó en uso el `0001` antes de que `Expediente-Rules.md` 1.0 existiera: carátula de cinco campos, tipo entre comillas invertidas en la cabecera, evidencia sin las tres líneas de A9 en las piezas anteriores al folio 016. Re-medido con A1 a A12: **A1, A10 sobre este expediente, A11 y A12 dan cero**; A2 a A5 y A9 marcan lo que la forma provisoria no tiene. Su migración a la forma normada, o su `archivo`, es el pase del folio 016. A10 marca además `SDD/Expedientes/0001-Migracion-Normativa-A-13.16`, cita a un expediente de destino que este repositorio nombra como texto: ya estaba antes de esta entrada.

### No agregado — `Conocimiento/Knowledge-Bundle-JS.md`

Una copia del documento había quedado sin seguimiento en el árbol de `Conocimiento/`, idéntica byte a byte a la de `OUTPUTs/`. **Se retira sin commitear** por decisión del Product Owner del 2026-09-13 (folio 016 §1): el tool-prompt pidió que el documento no se incorporara al framework para no acoplar en los dos sentidos, y el propio documento lo declara en §9.
