...
- ¿Hay proyectos de código de más de un ecosistema? Si un proyecto toma como entrada de su construcción el artefacto que genera otro, sin referenciarlo —el caso normal entre ecosistemas distintos—, esa dependencia es un **insumo de construcción**: se marca en la columna de dependencias y un solo consumidor lo genera (`Intake-Rules.md` §4).
...
Con proyectos de código de más de un ecosistema, el perfil se declara **una vez por ecosistema**, y cada proyecto toma el suyo (`PRODUCT-MANIFEST-template.md` §1.2, con un ejemplo).
...
Instrucción: Describir cómo se materializan los samples según el tipo D8 de cada unidad de entrega, que es el eje que lo lleva (§13.1). Cuando un sample ejercita un proyecto de código en particular, se nombra el proyecto, y el tipo sigue siendo el de la unidad de entrega que ese proyecto compone. Cada sample autocontenido, ejecutable, con su nivel de complejidad y con su forma de entrada al agrupador de la solución de código (`Rules-Examples.md` §3.6).

| Tipo D8 de la unidad de entrega | Qué hay en `/samples` |
|---|---|
...
### §17.P.13 Conocimiento a aplicar
Instrucción: **Opcional.** Documentos del catálogo `Conocimiento/` del framework que este proyecto de código debe aplicar, citados por su **alias**. Se usa cuando querés forzar la carga de un conocimiento que la condición del índice no dispararía sola, o dejar registro de intención de producto. Si no aplica ninguno, escribir `Ninguno`.

Una fila por alias citado:

| `alias` | Motivo de la cita | Alcance |
|---|---|---|
| [Alias tal como figura en `Conocimiento/Index-Knowledge.md`] | [Por qué este proyecto de código lo aplica] | [Todo el proyecto de código, o la parte que alcanza] |

Reglas de esta subsección:
- **Un alias que no resuelve contra el índice es error bloqueante de validación de intake.** Se verifica una sola vez, antes de la Fase A.
- Citar un conocimiento cuya condición de carga no dispara **no es un apartamiento y no lleva ADR**: la condición del índice es un disparador por defecto, no una obligación. El campo `Motivo` alcanza como registro de intención.
- No hace falta citar lo que la condición del índice ya dispara sola. Citarlo igual no es un error, sólo es redundante.
- Qué categoría o subagente recibe cada documento lo declara el índice, no esta tabla.

Preguntas guía: (*) ¿Hay una arquitectura, un template o una convención de la casa que este proyecto de código deba seguir? (*) ¿Está capturada en `Conocimiento/`? ¿Con qué alias?

Lo que NO va en el bloque técnico:
- Necesidades de negocio (van a la Parte A).
- ADRs formales (viven en `05-Arquitectura-Tecnica/`).
- Conocimiento nuevo: esta subsección **cita** documentos existentes del catálogo, no los crea. Un conocimiento que todavía no está capturado se releva aparte, contra `Rules-Base-Conocimiento.md`.
...
- [ ] §17 está completo para cada unidad de entrega **vigente** de §13.1 (sus **dos** tablas de identidad + P.1 a P.12; P.13 es opcional y admite `Ninguno`). **No hay un bloque §17 por proyecto de código**: los proyectos aparecen dentro del bloque de cada entrega que componen.
...
| §14 estilo de producto | `05-Arquitectura-Tecnica/` (vista de producto) | `Arquitectura-Unidad-Entrega.md` |
...
| §17 P.13 conocimiento a aplicar | No genera documento: **alimenta el despacho** de la categoría o el subagente que el índice declare | Insumo adicional en la lista de insumos obligatorios |
...
| 3.5 | 2026-08-23 | **Nueva `§17.P.13`, la cita de conocimiento** (framework 13.2). Es la puerta de entrada del catálogo `Conocimiento/` a una corrida: una tabla de alias con su motivo y su alcance, por proyecto de código. **Opcional y con `Ninguno` como valor válido**: sin citas, el despacho se arma exactamente como antes. Declara sus cuatro reglas, entre ellas que **un alias que no resuelve es bloqueante** y que **citar un conocimiento cuya condición no dispara no es un apartamiento y no lleva ADR**, porque la condición del índice es un disparador por defecto y no una obligación. Se agrega **al final del bloque repetible, sin renumerar** ninguna subsección existente, de modo que la plantilla sube **minor** y no arrastra al conjunto a major. El checklist de §19 y la tabla de mapeo a documentación se actualizan en consecuencia. |
| 3.6 | 2026-09-13 | **§16.1 pedía los samples «según el tipo D8 de cada proyecto de código»**, con el modelo anterior a la 3.0, que hizo del D8 un atributo de la unidad de entrega: quien completara la sección siguiendo la instrucción declaraba D8 por proyecto, contra §13.2 del mismo documento. Pasa a pedirlo **de la unidad de entrega**, a nombrar el proyecto de código sólo cuando el sample ejercita uno en particular, y a declarar la entrada de cada sample al agrupador de la solución (`Rules-Examples.md` §3.6); la cabecera de su tabla lo dice. **§13.2 suma la pregunta por el proyecto de código de otro ecosistema** y la marca de insumo de construcción de `Intake-Rules.md` §4, y **el perfil de convención** suma la capitalización y su declaración por ecosistema. **Corregido de paso**: la fila 3.5 de este control de cambios estaba antes de la 3.0; se reordena sin cambiar su texto. Sube minor, con el precedente de la 3.1: corrige un residuo del modelo de dos ejes, y la marca nueva sólo alcanza a los productos que ya tenían el caso. | Framework SDD (proyecto de otro ecosistema) |
