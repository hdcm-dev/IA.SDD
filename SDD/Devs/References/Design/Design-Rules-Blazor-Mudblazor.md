...
| --- | --- | --- | --- |
| 1.0 | 2026-06-19 | Versión inicial. Mapeo del catálogo genérico a MudBlazor v9: theme tipado, render Interactive Server, patrones → componentes, estados, iconografía SVG con íconos custom, accesibilidad. | AG-00030 UX/UI + Frontend Architect |
| 1.1 | 2026-06-20 | Mapeo de los patrones de la extensión `Design-Rules-Config-Esquema` a componentes MudBlazor (nueva §4.1): campo dirigido por descriptor, ayuda contextual, divulgación progresiva, presets, explicación en palabras, indicador de simulación y ranura del asistente; estado `info` vía `Color.Info`/`Palette.Info`; previsualización de la propuesta con `MudDialog`. | AG-00030 UX/UI + Frontend Architect |
| 1.2 | 2026-07-18 | Mapeo de los patrones de las extensiones `Design-Rules-Primer-Arranque`, `Design-Rules-Acceso-Monousuario` e `Design-Rules-Identidad-De-Version` a componentes MudBlazor (nueva §4.2): shell partido sin `MudAppBar`/`MudDrawer`, tarjeta de aprovisionamiento y de acceso, redirección con `MudProgressLinear` y `replace: true`, banda de resultado con `MudAlert`, barra de identidad, orientación posterior, sello de versión y detalle de diagnóstico. Nota de fidelidad sobre formularios de identidad por POST a endpoints (fuera del circuito interactivo) con `AntiforgeryToken` y `autocomplete` declarado. §11 registra las extensiones mapeadas. | AG-00030 UX/UI + Frontend Architect |
