...
**Carpeta target (por unidad de entrega):** `SDD/Docs/Unidades-Entrega/<Nombre-Unidad-Entrega>/11-Documentacion/`
...
- [1. Especialidad asignada](#1-especialidad-asignada)
  ...
  - [1.2 Variantes según tipo de unidad de entrega (8 valores D8)](#12-variantes-según-tipo-de-proyecto-8-valores-d8)
...
- [2. Documentos que produce esta categoría](#2-documentos-que-produce-esta-categoría)
  ...
  - [2.2 Cuerpo integrador (nivla unidad de entrega)](#22-cuerpo-integrador-nivel-proyecto)
  - [2.3 Cuerpo mantenedor (nivla unidad de entrega)](#23-cuerpo-mantenedor-nivel-proyecto)
  - [2.4 Cuerpo operador (nivla unidad de entrega)](#24-cuerpo-operador-nivel-proyecto)
...
| --- | --- | --- |
| **Integrador** | Consume la unidad de entrega desde otra aplicación o sistema, sin conocer su interior | «¿Cómo lo uso?» |
...
| --- | --- | --- |
| **1 — Plan documental** | Pre-código, una vez derivado y confirmado el `PRODUCT-MANIFEST` | El índice del cuerpo documental: qué artefactos va a tener cada unidad de entrega, su rol de intervención y su estado inicial `Planificado`. Sin contenido redactado |
...
Con el manifiesto confirmado ya se conoce la composición del producto —qué unidades de entrega hay, de qué tipo es cada uno y qué rol cumple—, y eso alcanza para determinar qué documentos va a tener cada unidad de entrega sin redactar una línea. El Momento 1 se genera junto con el plan de generación que el orquestador presenta para aprobación, de modo que el usuario vea desde el principio qué documentación va a existir al final.
...
`Bitacora-Eventualidades.md` es el artefacto de captura, de nivel producto, con una entrada por eventualidad identificada `EVE-XXXXX`. Su estructura de campos vive en §4.2.
...
| Falla reproducible con síntoma observable en ejecución | `Runbook-Operacion` → nueva entrada `OPS-XXXXX` |
| Falla que golpea a quien integra la unidad de entrega desde afuera | `Troubleshooting` → nueva entrada `ISSUE-XXXXX` |
...
Cuando la unidad de entrega expone una API pública o un CLI, el rol incluye además la generación o curación de referencia autogenerada desde código (XML docs, OpenAPI, help embebido), garantizando paridad estricta entre la API publicada y la documentada.
...
El orquestador lee esta tabla y, según el `tipo_unidad_entrega` de la unidad de entrega en curso (leído del manifiesto), decide qué variante de especialidad se activa. **La categoría siempre se genera**: lo que varía es qué cuerpos se materializan dentro de ella, según §2.5.
...
- **No asumir** que el lector conoce la unidad de entrega ni la documentación previa.
...
Se generan una sola vez para todo el producto, bajo `SDD/Docs/Producto/11-Documentacion/`. En el caso degenerado de un producto de un solo unidad de entrega, van directamente bajo `SDD/Docs/11-Documentacion/`, sin subnivel.
...
| `Guia-Despliegue.md` | Operador | Procedimiento de despliegue por topología: prerrequisitos, orden de arranque entre unidades de entrega, cómo se resuelven entre sí, configuración por ambiente, verificación paso a paso y rollback |
| `Bitacora-Eventualidades.md` | Operador, Mantenedor | Registro de las situaciones no previstas que aparecieron durante la construcción, el despliegue y la operación, con síntoma, causa, resolución e intentos descartados. Cada entrada se identifica `EVE-XXXXX` y se triaja hacia un documento permanente, según §0.6 |
...
| `AGENTS.md` (emitido en la **raíz del repositorio destino**) | Agentes de IA | Contrato de contexto para agentes: cómo se construye la unidad de entrega, cómo se corren los tests, convenciones de código, comandos de validación, límites de intervención, y punteros a los documentos de 11 por intención |
...
### 2.2 Cuerpo integrador (nivla unidad de entrega)
...
| `Troubleshooting.md` | How-to orientado a diagnóstico | Errores comunes con identificador `ISSUE-XXXXX`, diagnóstico paso a paso, logs útiles y cómo reportar un bug |
...
### 2.3 Cuerpo mantenedor (nivla unidad de entrega)
...
| `Guia-Extension.md` | Puntos de extensión publicados con identificador `EXT-XXXXX`, contrato de cada uno y ejemplo de registro. Solo cuando la unidad de entrega declara extensibilidad en 05 |
...
### 2.4 Cuerpo operador (nivla unidad de entrega)
...
| `Runbook-Operacion.md` | Procedimientos de operación: arrancar, parar, reiniciar, verificar salud, leer logs y qué patrón buscar, métricas relevantes y sus umbrales. Incluye los incidentes conocidos con identificador `OPS-XXXXX`, cada uno con síntoma, diagnóstico paso a paso y resolución |
...
| --- | --- | --- | --- |
| `OPS-XXXXX` | Incidente operativo conocido | `Runbook-Operacion` | Esta regla |
| `EXT-XXXXX` | Punto de extensión publicado | `Guia-Extension` | Esta regla |
| `EVE-XXXXX` | Eventualidad capturada | `Bitacora-Eventualidades` | Esta regla, §0.6 |
...
- **Downstream**: dentro de la cadena SDD esta categoría no tiene downstream. El README maestro de la unidad de entrega la referencia como punto de entrada.
...
- **Menor**: incorporaciones que no rompen lo vigente. Un `OPS-XXXXX` nuevo, un punto de extensión más, una variable de entorno agregada.
...
3. Diagrama de contenedores en Mermaid: las unidades de entrega que la componen y cómo se comunican entre sí.
4. Tabla de unidades de entrega: nombre, tipo D8, qué hace en una línea, dónde vive el código, de qué depende.
...
4. Orden de arranque entre unidades de entrega cuando el comando único no alcanza, derivado del grafo de dependencias del manifiesto.
...
**`Troubleshooting.md` (How-to de diagnóstico).** Errores comunes con `ISSUE-XXXXX` en tabla síntoma/causa/solución · diagnóstico paso a paso con comandos que permitan confirmar la causa antes de aplicar la solución · logs útiles con nivel y patrón a buscar · cómo reportar un bug con plantilla y datos mínimos.
...
Una sección por punto de extensión, identificada `EXT-XXXXX`: qué permite extender, contrato de la interfaz o del punto de enganche, ejemplo de registro completo, límites de lo que la extensión puede hacer, y enlace al sample de 10 que lo demuestra cuando existe.
...
5. **Incidentes conocidos**: una entrada por incidente, identificada `OPS-XXXXX`, con síntoma observable, diagnóstico paso a paso y resolución. El diagnóstico es una secuencia de comandos que permite confirmar la causa antes de aplicar la solución, no una hipótesis.
...
| Usar la unidad de entrega desde mi aplicación | Integrador | `Guia-Onboarding-Developer.md` |
...
| --- | --- | --- | --- |
| Documentos sin sufijo de versión | Pierde trazabilidad entre release y documentación | Sufijo `.md` obligatorio, salvo la excepción declarada de `AGENTS.md` | [enumerable] |
| Nombre de archivo hardcodeado con un sistema concreto | Ata el template a un stack particular | Parametrizar con slug genérico | [enumerable] |
| Mezcla de cuadrantes Diátaxis en el cuerpo integrador | Un tutorial con explicaciones largas se vuelve ilegible; una referencia con narrativa pierde rigor | Declarar `Tipo Diátaxis` en la cabecera y respetarlo | [interpretativo] |
| **Producir una versión «para humanos» y otra «para la IA»** | Divergen en el segundo cambio y nadie sabe cuál vale | Un documento, dos caras. Ante divergencia se corrige, nunca se bifurca | [enumerable] |
| **Confundir el eje de rol con el eje de naturaleza del lector** | Produce cuerpos duplicados y organización incoherente | El rol organiza los cuerpos; la naturaleza se resuelve dentro de cada artefacto | [interpretativo] |
| **Recorrido de código que cita rutas inexistentes** | Manda al mantenedor a buscar algo que no está; es peor que no tener el documento | Toda ruta se verifica en el ensayo automatizado de cada corte | [interpretativo] |
| **Redefinir la estrategia de testing en la guía de contribución** | Dos estrategias que divergen; el mantenedor no sabe cuál rige | 11 cita a 08 por referencia; no la copia | [interpretativo] |
| **Redefinir la política de ambientes en la guía de despliegue** | Misma falla, contra 09 | 11 documenta el procedimiento verificado; la política vive en 09 | [interpretativo] |
| **Duplicar el código de un sample dentro de la documentación** | El sample evoluciona y la copia queda vieja | Enlazar al `ejemplo-XXXXX` de 10 por identificador | [interpretativo] |
| **Documentar la implementación interna en conceptos fundamentales** | Confunde el rol de intervención y duplica 05 | 11 documenta el modelo mental visible al integrador; la implementación vive en 05 | [interpretativo] |
| **Criterio de verificación redactado como prosa** | «Comprobar que el servicio arrancó bien» no es evaluable por un agente | Comando verbatim y aserción: exit code, respuesta HTTP, patrón de log | [interpretativo] |
| **Redactar todo el cuerpo al cierre** | Describe un sistema que el redactor ya no recuerda haber construido, y llega tarde para corregir el diseño | Los tres momentos de §0.3, con la cadencia de §0.4 | [interpretativo] |
| **Declarar cerrado un corte con documentos afectados sin revisar** | La deuda documental se vuelve invisible y se acumula | Definition of Done del sprint incluye la actualización de 11 | [enumerable] |
| **Cerrar una eventualidad sin destino asignado** | El conocimiento se pierde igual que si no se hubiera registrado | Triaje obligatorio de §0.6; `No absorbida` es un destino válido, «sin destino» no | [enumerable] |
| **Que el agente que documentó apruebe su propio ensayo de entrega** | Conoce el sistema porque acaba de documentarlo; la prueba no prueba nada | El ensayo humano es un gate de confirmación humana | [interpretativo] |
| **Salirse de la documentación durante el ensayo y no registrarlo** | Se pierde exactamente el hallazgo que el ensayo existe para producir | El momento en que hay que salirse es el hallazgo | [interpretativo] |
| Glosarios duplicados por documento | El mismo término con definiciones distintas en cada archivo | Un único glosario referenciado desde todos | [interpretativo] |
| Cobertura cero de troubleshooting | El integrador se traba ante el primer error y abandona | Mínimo cinco entradas `ISSUE-XXXXX` basadas en errores reales | [interpretativo] |
| Documento que nunca declara su fecha de revisión | No se puede distinguir lo vigente de lo abandonado | `last_review` obligatorio en el frontmatter, actualizado en cada corte | [interpretativo] |
---
...
- ¿Los tres roles tienen su cuerpo, o alguno quedó implícito porque la unidad de entrega «no lo necesita»?
...
- ¿Alguien que nunca vio la unidad de entrega podría ubicar dónde se valida una entrada leyendo solo este documento?
...
- [ ] [interpretativo] La categoría existe para la unidad de entrega. Es obligatoria para los ocho tipos D8.
- [ ] [interpretativo] El cuerpo mantenedor está materializado, con `Recorrido-Codigo` y `Guia-Contribucion` como mínimo.
- [ ] [interpretativo] Los cuerpos integrador y operador se materializaron o se omitieron según la tabla de §2.5, y toda omisión está registrada en `Decisiones-Proyecto.md`, con ADR cuando el gating los declaraba obligatorios.
- [ ] [enumerable] Existen los artefactos de nivel producto bajo `Producto/11-Documentacion/`, o bajo `SDD/Docs/11-Documentacion/` en el caso degenerado.
- [ ] [enumerable] Existe `README.md` de la sección con su matriz de ruteo y su tabla de estado del cuerpo documental.
- [ ] [enumerable] Ningún artefacto lleva sufijo de versión en el nombre; cada uno declara su versión en la cabecera, salvo `AGENTS.md`, que se deriva del contrato versionado.
- [ ] [interpretativo] Ningún nombre de archivo hardcodea un sistema comercial concreto.

**Doble audiencia**

- [ ] [interpretativo] Todo documento tiene frontmatter YAML con `doc_id`, `doc_type`, `title`, `status`, `rol_intervencion`, `owner`, `last_review` y `traces`.
- [ ] [interpretativo] Todo documento abre con resumen ejecutivo de tres a cinco oraciones.
- [ ] [interpretativo] No existen versiones paralelas del mismo contenido segmentadas por tipo de lector.
- [ ] [interpretativo] Los criterios de éxito están expresados como aserción evaluable, no como prosa.
- [ ] [interpretativo] Los comandos son verbatim y copy-paste, con su salida esperada textual.
- [ ] [interpretativo] Las rutas son absolutas desde la raíz del repositorio.
- [ ] [interpretativo] Los documentos que definen un proceso ejecutable incluyen su bloque `entradas` / `salidas` / `validaciones`.

**Fronteras y trazabilidad**

- [ ] [interpretativo] Ningún documento redefine la estrategia de testing de 08, la política de ambientes de 09 ni las vistas de 05: las citan.
- [ ] [interpretativo] Ningún documento duplica el código de un sample de 10; los enlaza por identificador.
- [ ] [interpretativo] Cada documento del cuerpo integrador y del cuerpo mantenedor tiene al menos un enlace a 05.
- [ ] [interpretativo] Cada documento incluye su sección de referencias cruzadas con tres a cinco enlaces.
- [ ] [interpretativo] No hay documentación dirigida al usuario final no técnico.

**Recorrido de código**

- [ ] [interpretativo] Todas las rutas citadas en `Recorrido-Codigo` existen en el repositorio.
- [ ] [interpretativo] Cada componente declarado en 05 tiene su fila en el mapa arquitectura a repositorio.
- [ ] [interpretativo] El recorrido del flujo principal nombra archivos concretos en orden.

**Documentación viva**

- [ ] [interpretativo] El plan documental del Momento 1 existe y se corresponde con lo producido.
- [ ] [interpretativo] Todo documento declara `last_review` con fecha posterior al último corte que lo afectó.
- [ ] [interpretativo] Ningún documento afectado por el corte quedó sin revisar antes de declararlo cerrado.
- [ ] [interpretativo] Los documentos sin revisar desde hace más de dos cortes están marcados `Potencialmente desactualizado` en el README.
- [ ] [interpretativo] El `AGENTS.md` se emitió en la primera corrida del Momento 2 y se refrescó en la última.
- [ ] [interpretativo] `Contrato-Agentes.md` y `AGENTS.md` no divergen.

**Ensayo de entrega**

- [ ] [interpretativo] El ensayo automatizado se corrió en el corte, y todo comando documentado ejecuta sin error.
- [ ] [interpretativo] Hay al menos un guion de ensayo definido por rol de intervención aplicable.
- [ ] [interpretativo] El ensayo humano se corrió y fue aprobado antes de cerrar el Momento 3.
- [ ] [interpretativo] Cada trabada del ensayo tiene documento y sección de destino asignados, y se resolvió antes de cerrar el corte.

**Bitácora**

- [ ] [enumerable] Existe `Bitacora-Eventualidades.md` de nivel producto.
- [ ] [interpretativo] Cada `EVE-XXXXX` tiene sus nueve campos completos, incluido `intentos_descartados`.
- [ ] [interpretativo] Ninguna eventualidad quedó cerrada sin destino asignado; las `No absorbida` tienen su motivo.
- [ ] [interpretativo] Ninguna eventualidad quedó abierta más de un corte sin triaje.

**Voz y forma**

- [ ] [interpretativo] Ninguna sección abre reformulando su encabezado.
- [ ] [interpretativo] No hay muletillas de relleno del listado de §4.6.
- [ ] [interpretativo] Los diagramas son Mermaid y son sintácticamente correctos.
- [ ] [interpretativo] Los enlaces internos resuelven.
...
momento: 2
traces: [ADR-002, ADR-004, CU-00001]
---

# Recorrido de código del servicio de pagos

**Unidad de entrega:** Servicio-Pagos
...
## Recorrido del flujo principal: confirmar un pago (CU-00001)
...
Origen: `EVE-00003` de la bitácora de eventualidades. Se detectó en el primer despliegue
containerizado; ninguna vista de arquitectura lo anticipaba.
````

### 7.3 Entrada de bitácora `EVE-00003`
...
destino: >
  ...
  Runbook-Operacion.md, entrada OPS-00007 «El servicio no registra lecturas».
```

Los tres fragmentos son ilustrativos. Cada unidad de entrega adapta el dominio respetando la estructura, la nomenclatura y el contrato de doble audiencia.
...
```text
Sos un {{ESPECIALIDAD-VARIANTE-11}} responsable del cuerpo documental de entrega de la unidad de entrega {{NOMBRE_UNIDAD_ENTREGA}}.

Momento en curso: {{MOMENTO}} (1 plan documental | 2 actualización incremental | 3 consolidación de cierre).

Insumos:
- PRODUCT-INTAKE: {{path}} (§13 tipo D8, §16 estructura de repo, §17 técnica por unidad de entrega)
- Upstream: 02 (CU y modelo de datos), 05 (arquitectura, contratos, ADR, puntos de extensión), 08 (estrategia de testing, que citás y no redefinís), 09 (política de despliegue, que convertís en procedimiento verificado), 10 (samples y contratos de verificación VER-XXXXX, que enlazás y no duplicás).
...
| 5.5 | 2026-08-22 | §3.2 decía que sus identificadores son únicos en el producto «como el resto de los identificadores **del framework**», mezclando los **dos ámbitos** que `Root-Rules.md` §9.1 acaba de separar: `OPS`, `EXT`, `EVE`, `ISSUE` y `VER` son familias **del producto**. Sube **patch**: corrige una atribución sin cambiar la regla. |
