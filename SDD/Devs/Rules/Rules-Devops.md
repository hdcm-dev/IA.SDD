...
| Insumo de construcción omitido en silencio | La solución se construye bien donde falta la cadena de herramientas del ecosistema productor y entrega el consumidor sin su artefacto; el defecto aparece recién en ejecución | La construcción falla y nombra la cadena que falta; el ambiente que no la tiene usa un modo de construcción explícito que recibe el artefacto de otra etapa (§4.9, punto 4) | [interpretativo] |
| Más de un generador del mismo insumo de construcción | Cada lugar del pipeline que lo genera produce su propia copia, y un cambio del productor llega a una sola | Un único generador, el que declara el manifiesto; el resto de los lugares toma su salida (§4.9, punto 4) | [interpretativo] |
...
4. Coordinación inter-proyecto. Por cada arista de dependencia, cómo el proyecto de código consumidor obtiene el artefacto del productor: por referencia al paquete publicado (el redistribuible se publica primero) o por build conjunto en el repositorio. Declara la política para cada dependencia. Cuando la arista es un **insumo de construcción** (`Intake-Rules.md` §4), declara además su **único generador**. Y **cuando la construcción de la solución necesita la cadena de herramientas de más de un ecosistema** —por un insumo de construcción o dentro de un mismo proyecto de código—, declara en qué ambientes esa cadena falta y cómo se construye ahí: con un **modo de construcción explícito y nombrado** que omite la generación y recibe el artefacto ya generado por otra etapa. Sin ese modo, la construcción donde falta la cadena **falla con un error que la nombra**; nunca termina bien entregando el consumidor sin su insumo.
...
| 6.2 | 2026-09-13 | **§4.9, punto 4, suma el insumo de construcción y la cadena de herramientas ausente.** La coordinación inter-proyecto ya admitía el build conjunto en el repositorio y no decía qué pasa cuando la construcción de la solución necesita la cadena de herramientas de más de un ecosistema y se corre donde una falta. Declara el **único generador** de cada insumo de construcción (`Intake-Rules.md` §4) y un **modo de construcción explícito y nombrado** para los ambientes sin la cadena, con la propiedad que lo justifica: sin ese modo la construcción **falla y la nombra**, nunca entrega el consumidor sin su insumo. §4.8 suma **dos anti-patrones** `[interpretativo]`: el insumo omitido en silencio y los varios generadores del mismo insumo. Agnóstico de stack. Sube minor: un pipeline de producto de un solo ecosistema cumple igual. |
