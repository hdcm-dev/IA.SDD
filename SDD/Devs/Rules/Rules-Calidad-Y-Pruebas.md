...
Esta categoría es **obligatoria para los ocho tipos D8**. Ninguna unidad de entrega está exento de declarar su estrategia de calidad, su pirámide de testing, su matriz de cobertura y su Definition of Done. El nivel de exhaustividad y la mezcla de artefactos cambia por tipo, pero la categoría siempre existe.
...
| --- | --- | --- | --- |
| Cobertura como número global sin distinguir capas | "85 % cobertura" puede esconder 100 % en getters y 30 % en dominio | Reportar cobertura por capa con umbrales diferenciados | [enumerable] |
| Tests sin trazabilidad a CU, RN o NFR | No se puede responder qué requisito valida cada test | Cada TC referencia al menos un CU, RN o NFR | [enumerable] |
| Snapshot test sin política de regeneración controlada | Los snapshots se "regeneran" para que pasen, perdiendo su valor | Regeneración requiere PR con justificación y revisión | [enumerable] |
| E2E pesado sustituyendo unit tests | Suite lenta, frágil y de difícil diagnóstico | Mantener proporción declarada en la pirámide | [interpretativo] |
| DoD sin métricas verificables | "El rendimiento es aceptable" no se puede chequear en CI | Cada criterio DoD tiene una operación mecánica que lo valida | [enumerable] |
| Falta de prueba de regresión cuando se cierra un bug | El bug puede volver y nadie se entera | Todo bug cerrado genera al menos un TC nuevo o extiende uno existente | [enumerable] |
| Tests sin assert | Ejecutan código pero no verifican nada | Cada test tiene al menos una verificación explícita | [enumerable] |
| Coverage como meta única | 100 % de cobertura con tests triviales no mejora la calidad | Priorizar edge cases, mutation testing y trazabilidad sobre el número | [enumerable] |
| DoD redefinida en cada sprint plan | Inconsistencia y dilución del concepto | DoD canónica única referenciada desde los sprint plans | [enumerable] |
| Matriz desactualizada | Dice "Pendiente" pero hay 50 tests implementados | Actualizar la matriz al cierre de cada sprint | [interpretativo] |
...
- [ ] [interpretativo] **`Definition-Of-Done.md` es fuente única de la capa de verificación de la condición de terminado**, y **refina** la capa de acuerdo del equipo que `00-Contexto/Acuerdo-Equipo.md` §5 declara a nivel producto: la referencia y agrega los criterios que dependen de qué se entrega —pirámide de testing, quality gates, matriz de cobertura—. Las dos capas tienen dueños y niveles distintos y ninguna copia a la otra. Ningún artefacto de ninguna categoría la redefine ni enumera sus criterios: la referencian. Alcanza a los planes de sprint, al `Acuerdo-Equipo.md` de la categoría 00, a la condición de listo de la 06 y a cualquier otro artefacto que la cite. Mientras esta categoría no se haya emitido, la 06 y la 07 referencian la **capa de acuerdo del equipo** de la Fase A, que sí existe, y declaran con la forma de `Root-Rules.md` §12.1 que la capa de verificación llega en la Fase E.
...
Salida: SDD/Docs/Unidades-Entrega/{{NOMBRE_UNIDAD_ENTREGA}}/08-Calidad-Y-Pruebas/<estructura>.
```
...
| 1.3 | 2026-07-19 | Incorporación de `Matriz-Sensado-Deriva.md` a la tabla maestra de §2.1, condicionada al flag `requiere_maqueta`, y del criterio de aceptación correspondiente en §6. La matriz la emite AG-00031 al cerrar la Fase B2 y AG-00080 la incorpora a la estrategia de testing resolviendo el método de verificación de cada fila. Su regla completa vive en `Deriva-Rules.md`. |
| 1.4 | 2026-07-26 | Intercambio de categorías 10 ↔ 11: la colaboración multi-especialidad de §1.3 pasa a nombrar a AG-00110 Technical Writer / Documentation Lead y a la categoría 11 como destino de la guía de testing del repositorio. Normalización del vocabulario de actores. |
