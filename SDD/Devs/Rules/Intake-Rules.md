...
- Técnica (Parte C): §17 completo por cada unidad de entrega **vigente** de §13.1 —no por proyecto de código—, con las dos tablas de identidad que la plantilla declara y los P bloqueantes de §2. **§17.P.13 es opcional**: admite `Ninguno` y su ausencia no bloquea.
- Cita de conocimiento (§17.P.13): **todo alias citado resuelve** contra la columna `Alias` de `Conocimiento/Index-Knowledge.md` del framework, y su fila declara `Estado` = `Vigente`. Un alias que no resuelve, o que resuelve a una fila `Superado`, es **bloqueante**. Se verifica acá, una sola vez, y no en runtime: detectado en la validación cuesta una corrección del intake; detectado en la Fase B, cuesta la Fase A entera. Si `Conocimiento/` está vacía o no tiene índice, la única cita válida es `Ninguno`.
...
- Bloqueante: campo de §2, falla de derivación del manifiesto de §4, incoherencia cross-parte de §5, o **alias de §17.P.13 que no resuelve contra el índice de `Conocimiento/`**. Detiene la cadena. Entra acá la **discrepancia de conteo no declarada** dentro de un escenario de la Parte D.
