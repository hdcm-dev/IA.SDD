...
| **Mesa de evaluación** | `Mesa-Rules.md`, con su contrato de entrada, su composición por señal, su jurado y su lista cerrada de escaladas | La **convoca en M1** si la invocación no llega desde la reanudación, y **verifica su registro** si llega con él. No la redefine. **La vuelve a convocar sólo por la mesa antes de la detención de `Master-Prompt.md` §8.1**: una vez por corte de M4, o una vez por fase en M2, M3 y M5, sobre los tropiezos que no bloquean, y nunca sobre sus propias escaladas |
...
**Correcciones manuales.** Si al abrir un documento el subagente encuentra contenido que el snapshot no refleja, se detiene y lo devuelve como ambigüedad según `Master-Prompt.md` §9, sin editar. **Antes de tratarla como corrección manual, el orquestador calcula su origen del hecho contra la base de la corrida** (`Master-Prompt.md` §8.1): el snapshot se toma al despachar, y un documento que la migración estructural ya movió o fusionó en esta misma corrida lo refleja, de modo que la diferencia puede ser de la corrida y no del humano. Si es de la corrida, se cierra con la autocorrección sobre el conjunto y no se pregunta. Si es ajena, el orquestador enumera las diferencias, declara cómo las interpretó y espera confirmación antes de propagar. Es la regla de `Migracion-Rules.md` §4.2 y el mismo patrón que el criterio de re-ejecución del orquestador de generación. **Si la diferencia ajena no se cierra con el árbol y no bloquea el corte, no se pregunta de a una**: entra al lote del corte y pasa por la mesa antes de la detención de `Master-Prompt.md` §8.1.
...
| Plan de migración normativa | El contrato entre el orquestador de generación y el de migración: lo emite la salida A de la reconciliación y lo consume M1 |
...
| 2.10 | 2026-09-12 | **M4 calcula el origen del hecho antes de tratar una diferencia como corrección manual** (`Master-Prompt.md` §8.1). Es la segunda copia de la cláusula del snapshot de `Master-Prompt.md` §8: el snapshot se toma al despachar, y un documento que la migración estructural ya movió o fusionó en esta misma corrida lo refleja, de modo que la diferencia puede ser de la corrida y no del humano. Sube **minor**. |
| 2.11 | 2026-09-13 | **La mesa se vuelve a convocar a mitad de la migración, por la condición de `Master-Prompt.md` §8.1** (framework 13.18, reporte `31`). La tabla de §1 decía «no la reconvoca dos veces»: se **modifica de forma declarada**, porque el pedido del humano es que un tropiezo no detenga sin antes pasar por una mesa, y la frase lo prohibía. Se acota a **una mesa por corte de M4** —el corte ya tiene audit y confirmación, y «lote de fase» no era decidible en M4— y **nunca sobre sus propias escaladas**. **M4, correcciones manuales**, cablea la diferencia ajena al lote del corte. Sube **minor**. | Intervención del reporte `31` |
