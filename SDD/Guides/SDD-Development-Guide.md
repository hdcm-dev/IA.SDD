...
| **Cómo se audita** | Las catorce comprobaciones de §VI.3 están definidas **sobre texto**. Ninguna contempla probar código, de modo que el código entraría sin compuerta |
...
### III.11 Agregar un documento de conocimiento

**Qué estás agregando.** Una caracterización de algo que el framework **no gobierna** —una arquitectura, un template, una convención de nomenclatura— para que un subagente pueda reproducirlo o consumirlo. Es **oficio**, no método.

**Es el único eje de extensión que no toca una sola regla.** Un documento de conocimiento se agrega a `Conocimiento/` y se registra en su índice, y con eso alcanza. Ninguna regla lo nombra: lo único que el framework fija es el formato del documento y el contrato del índice, en `Rules-Base-Conocimiento.md`.

**Archivos a tocar:** `Conocimiento/Knowledge-<Tema>.md` y su fila en `Conocimiento/Index-Knowledge.md`.

**La propiedad que no se puede romper.** **El framework tiene que seguir funcionando con `Conocimiento/` vacía.** No hay flag que la apague: sin documentos, el índice no tiene filas y ningún alias resuelve. Si una extensión hace que el framework deje de correr con la carpeta vacía, la capacidad dejó de ser una extensión y pasó a ser parte del método.

**Invariantes.** **La ofuscación es bloqueante**, con el mismo rigor que §III.6 exige para un modelo UX-UI y por el mismo motivo: `IA.SDD` es público. Un documento se captura de un proyecto real, así que ningún nombre de cliente, dato real o asset del origen puede sobrevivir. Lo que sí se conserva a propósito son las **convenciones de la casa** —prefijos de esquema, formas de nombre—, que son el conocimiento que el documento existe para transmitir.

**Qué obligaciones dispara.** Escribe en este repositorio, de modo que **es una intervención**: entrada en el `CHANGELOG.md`, copia del conjunto superado a `_legacy/` y nota de coherencia si alcanza a varios archivos. **`Conocimiento/` entra en el snapshot**, por el criterio de §VI.5 —sólo se excluye lo que no condiciona lo que el orquestador genera— y con el precedente de `Templates/`.

**Cómo verificar.** La lista de criterios de aceptación de `Rules-Base-Conocimiento.md` §6.1, los catorce ítems, con su marca `[enumerable]` o `[interpretativo]` declarada una por una.

**Frontera con §III.6, que se confunde fácil.** Los dos ejes producen reglas de diseño reusables y usan la misma convención de nombres agnósticos del dominio. **El criterio es el origen**: un modelo UX-UI se captura de una **maqueta que el framework generó** y se aplica en la Fase B2; un documento de conocimiento caracteriza **algo externo al framework**. Si viene de afuera, es este eje.

**Cómo llega el documento a un despacho.** Por dos vías, y la unión de las dos es lo que el orquestador inyecta: la **condición de carga** que la fila declara, evaluada contra el intake, los flags y el tipo D8 del proyecto de código en curso; y la **cita explícita** del alias en `§17.P.13` del intake. La mecánica vive en `Master-Prompt.md` §6 y §8. Cada documento va **únicamente** al despacho del **consumidor** que su fila declara —una categoría o un subagente de fase—, que es lo que evita que un alias citado se inyecte en los doce.

**Cuál de las dos vías conviene.** La condición es la vía por defecto y la buena: se carga sola, sin que el usuario tenga que saber que el catálogo existe. La cita explícita es para forzar lo que la condición no dispararía. **Una condición bien escrita hace innecesaria la cita**, y si un documento se cita siempre a mano, su condición está mal calibrada.

**Y si un subagente necesita algo que ninguna de las dos vías le trajo, lo pide.** `Master-Prompt.md` §9.1 define el ciclo: el subagente se detiene con un bloque `PEDIDO DE CONOCIMIENTO` que describe **la necesidad en prosa, no el alias** —no conoce el catálogo—, el orquestador despacha a **`AG-00980`** con el índice filtrado por consumidor, resuelve a rutas los alias que devuelva y reanuda con los documentos enteros. **El orquestador entrega; AG-00980 nunca**, para no crear la segunda fuente que §6 punto 1 prohíbe.

**Todo pedido es evidencia de una condición mal calibrada.** Si un subagente tuvo que pedir, esa era una condición que el índice debería haber disparado sola: el log de pedidos es lo que permite afinar el catálogo con su propio uso.
...
| 14 | **Afirmaciones de colisión con su medición**: toda afirmación de colisión o de no colisión de un término que la intervención **escribe** —en su nota, en su entrada del `CHANGELOG.md`, en el texto normativo— **o de la que parte** —la que trae su origen, la de su verificación previa— está en la nota con el comando reproducible que localiza sus ocurrencias por el contexto de lectura de `Vocabulario-Rules.md` §9.2, y su salida | Cero afirmaciones sin comando. **La recibida sin comando se reproduce antes de usarse**, y la que no se reproduce no se usa |
...
**Por qué la comprobación 14 existe, y por qué el punto 1 de §VI.3.1 no alcanzaba.** El punto 1 manda buscar
el término en todo el árbol, y rige **al cerrar** una intervención que cambia un concepto. **La afirmación de
colisión ocurre antes**: en la verificación previa, al elegir el nombre de algo que todavía no se escribió, o
llega ya hecha desde el origen. Medido: en un mismo trabajo, **seis afirmaciones de recuento, de sección o de
colisión** salieron sin su medición —una colisión afirmada sobre un término que no aparece en el archivo donde el campo iba a vivir
(`grep -o procedencia SDD/Devs/Rules/Root-Rules.md | wc -l` → 0),
una sección que no existía, un recuento de líneas leído como recuento de ocurrencias— y **ninguna la detectó
una comprobación**: todas, una relectura ajena. **Se leían igual de firmes que las ciertas**, y eso es lo que
la vuelve enumerable y no una cuestión de cuidado: la comprobación no mira si la colisión es real, mira que la
medición esté, y una medición con su comando se relee ejecutándola. **Alcanza a lo que llega del origen**
porque el caso medido es ése: la premisa verificada para un término se aplicó al siguiente sin volver a
medirla, que es el primado que `Vocabulario-Rules.md` §9.4 describe.
...
| 1.26 | 2026-08-23 | **Puesta al día contra el conjunto 13.0, que incorporó la capa de conocimiento sin que esta guía la mencionara.** Nueva **§III.11, el eje de extensión de un documento de conocimiento**, y es el único de los once que **no toca una sola regla**: se agrega el documento y su fila en el índice, y alcanza. Declara la propiedad que no se puede romper —**el framework tiene que seguir funcionando con `Conocimiento/` vacía**—, la ofuscación bloqueante por el mismo motivo que §III.6, las obligaciones de intervención que dispara, y que **`Conocimiento/` entra en el snapshot** por el criterio de §VI.5 y el precedente de `Templates/`. Escribe además la **frontera con §III.6**, que se confunde fácil: el criterio es **el origen**, no el parecido del resultado. Y declara el **estado de cableado**: el catálogo se puede poblar, pero el orquestador todavía no lo consume en una corrida. **§I.2 corrige dos defectos**: el recuento pasa de dieciocho archivos normativos y seis transversales a **diecinueve y siete**, y la fila de `Orchestrator/` anunciaba **tres** master-prompts describiendo sólo dos, así que entra `Master-Prompt-Reanudacion.md`. Sube **minor**: agrega un eje y corrige conteos sin derogar nada. |
| 1.27 | 2026-08-23 | **§III.11 corrige la declaración de estado de cableado, que la 13.2 volvió falsa.** Decía que el orquestador «todavía no lo consume en una corrida», y ya lo consume. En su lugar se describe **cómo llega el documento a un despacho**: por la unión de la **condición de carga** de su fila y la **cita explícita** del alias en `§17.P.13`, con el consumidor declarado decidiendo a qué despacho va. Se declara además cuál de las dos vías conviene —**la condición es la buena, y si un documento se cita siempre a mano su condición está mal calibrada**— y que **`AG-00980` existe pero todavía no se convoca**: falta el aviso en el despacho, y mientras tanto el conocimiento llega por el camino determinista. Sube **minor**. |
