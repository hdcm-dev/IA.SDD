...
| Ruta | Qué contiene |
| --- | --- |
| [`SDD/Devs/Rules/`](SDD/Devs/Rules/) | Los veintiún archivos de reglas constructivas: uno por categoría documental más nueve transversales. Es el corazón normativo del framework |
...
| [`Expedientes/`](Expedientes/) | **Registro, no conjunto normativo.** Los expedientes de caso del propio framework, uno por carpeta numerada, con su evidencia y la palabra del Product Owner. Los gobierna [`Expediente-Rules.md`](SDD/Devs/Rules/Expediente-Rules.md). Queda fuera de `_legacy/` y lo publicado no se reescribe |
...
Reglas transversales, que no gobiernan una categoría sino una capacidad del framework: [`Root-Rules.md`](SDD/Devs/Rules/Root-Rules.md) (layout canónico y README raíz de la salida), [`Intake-Rules.md`](SDD/Devs/Rules/Intake-Rules.md) (validación del documento de entrada), [`Maqueta-Rules.md`](SDD/Devs/Rules/Maqueta-Rules.md) (validación visual), [`Deriva-Rules.md`](SDD/Devs/Rules/Deriva-Rules.md) (sensado de deriva y evidencia verificable), [`Vocabulario-Rules.md`](SDD/Devs/Rules/Vocabulario-Rules.md) (vocabulario normativo: los seis términos del framework, los cuatro planos de identidad de un producto y el criterio de desambiguación léxica que gobierna los glosarios de la documentación generada) y [`Migracion-Rules.md`](SDD/Devs/Rules/Migracion-Rules.md) (migración normativa: cómo se lleva un destino generado con una versión anterior a la versión vigente preservando su contenido) y [`Expediente-Rules.md`](SDD/Devs/Rules/Expediente-Rules.md) (el expediente de caso: dónde queda escrito un caso que atraviesa corridas o repositorios, con su evidencia y la palabra del Product Owner).
...
| **D3** | Nombres | Título-Con-Guiones estricto en archivos y carpetas: cada palabra capitalizada, separadas por guion medio. Prohibidos espacios, acentos, eñes y caracteres especiales. Los identificadores llevan prefijo y **cinco dígitos uniformes** (`NB-00001`, `CU-00014`, `ADR-00007`, `US-00023` y equivalentes), y son **únicos en su ámbito declarado**: el **producto** para las familias que catalogan lo que el framework genera, y el **conjunto normativo vigente** para los roles del catálogo de especialidades y los subagentes de fase. Los dos **no colisionan en numeración** y **sí se citan a través de la frontera**. El ancho, los ámbitos, las familias alcanzadas, las exclusiones y la regla de titularidad viven en [`Root-Rules.md`](SDD/Devs/Rules/Root-Rules.md) §9 |
...
| Modificar una invariante D1 a D9 | Es el cambio de mayor impacto del framework: alcanza a los veintiún archivos de reglas, a los tres orquestadores y a toda la documentación ya emitida. Requiere decisión explícita del responsable y nota de coherencia |
...
| Tratar un caso del propio framework que atraviesa corridas o repositorios | Abrir su expediente en [`Expedientes/`](Expedientes/) con la forma de [`Expediente-Rules.md`](SDD/Devs/Rules/Expediente-Rules.md), y correr su compuerta S2 antes de cada push: este repositorio es público |
...
**Autosuficiencia.** Ningún archivo del conjunto normativo referencia otro repositorio. Es lo que permite clonarlo solo, moverlo o distribuirlo sin arrastrar dependencias. Al intervenirlo, esa propiedad se preserva: los estándares de industria se nombran, no se enlazan. **La excepción declarada es `Expedientes/`**, que no es conjunto normativo sino registro: un caso del framework ocurre sobre otros repositorios y los **nombra como texto, sin enlaces**. Los repositorios públicos se nombran; los privados se ofuscan (`Expediente-Rules.md` §4, S2). Sin `Expedientes/`, el repositorio se sigue clonando y usando solo.
